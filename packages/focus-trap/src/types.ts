import type { FocusableElement, TabbableCheckOptions } from '@nado/tabbable'
...
export interface FocusTrapOptions {
  ...
  /**
   * Функция для определения, безопасно ли отправлять фокус в ловушку фокуса
   * или нет.
   *
   * Он должен возвращать обещание, которое разрешается только после того, как все перечисленные «контейнеры»
   * способны получать фокус.
   *
   * Цель этого — предотвратить раннюю активацию ловушки фокуса на анимированных изображениях.
   * диалоги, которые появляются и исчезают. Когда диалог исчезает, происходит небольшая задержка
   * между активацией ловушки и фокусируемым элементом ловушки.
   */
  checkCanFocusTrap?: (containers: Array<FocusableElement>) => Promise<void>
  ...
  /**
   * По умолчанию: `истина`. Задерживает автофокус при активации ловушки фокусировки.
   * Это предотвращает захват элементами внутри фокусируемого элемента события,
   * вызвавшего активацию ловушки фокуса.
   */
...
export type Config = FocusTrapOptions & {
  document: Document
  delayInitialFocus: boolean
  isKeyForward: KeyboardEventToBoolean
  isKeyBackward: KeyboardEventToBoolean
