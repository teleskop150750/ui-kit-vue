...
describe('focus-trap', () => {
  beforeEach(() => cy.visit('./test/index.html'))

  function verifyCrucialFocusTrapOnClicking(focusedElSelectorOrAliasInTrap: string) {
  ...
  function verifyFocusIsNotTrapped(cyWrapper: Cypress.Chainable<JQuery<HTMLElement>>) {
    cyWrapper.should('be.focused')

    // focus can be transitioned freely when trap is unmounted
    let previousFocusedEl: Element

    cy.focused()
      .then(([lastlyFocusedEl]) => {
        previousFocusedEl = lastlyFocusedEl
      })
      .tab()

    cy.focused().should(([nextFocusedEl]) => expect(nextFocusedEl).not.equal(previousFocusedEl))
  }

  describe('demo: default', () => {
    it('H!', () => {
      cy.contains('h1', 'focus-trap demo')
    })
    ...
    it('traps focus tab sequence and allows deactivation by clicking deactivate button', () => {
      ...
      cy.get('@testRoot')
        ...
        .click()
      verifyFocusIsNotTrapped(cy.get('@lastlyFocusedElementBeforeTrapIsActivated'))
    })
  ...
  describe('demo: animated-trigger', () => {
    ;[true, false].forEach((returnFocus) => {
      it(`traps focus tab sequence and allows deactivation by clicking deactivate button, ${
        returnFocus ? 'focus returns to trigger on deactivate' : 'focus does NOT return to trigger on deactivate'
      }`, () => {
        ...
        if (returnFocus) {
          ...
          cy.get('@lastlyFocusedElementBeforeTrapIsActivated').should('be.not.focused')
        }
        ...
        verifyFocusIsNotTrapped(
          ...
          returnFocus ? cy.get('@lastlyFocusedElementBeforeTrapIsActivated') : cy.get('@deactivateButton'),
        )
      })
    })
  ...
  describe('demo: escape deactivates', () => {
    it('traps focus tab sequence and disallows deactivation by ESC after trap is activated', () => {
      ...
      cy.get('#escape-deactivates-option').as('testOption').should('be.checked')
  ...
  describe('demo: iene', () => {
    beforeEach(() => {
      ...
      cy.get('@testRoot').findByRole('link', { name: 'with' }).as('firstElementInTrap')
    })
    ...
    it('brings focus to a manually focused element if focus is lost outside the trap, when initialFocus=false', () => {
      ...
      cy.get('@testRoot').findByRole('link', { name: 'some' }).as('secondElementInTrap').should('not.be.focused')
  ...
  describe('demo: ifc', () => {
    ...
    it('specify element to be focused (even with attribute tabindex="-1") after focus trap activation, and use reverse tab order', () => {
      ...
      //  same as if we were able to do `cy.focused().tab()`, which means 4 tabs to get back
      //  to the 'first' button in the full sequence
      cy.get('@testRoot').findByRole('button', { name: 'first' }).as('firstTabbableElInTrap').tab({ shift: true }) // to deactivate
  ...
  describe('demo: ht', () => {
    ...
    it('focusing on only visually available (display is not "none" and visibility is not "hidden") elements', () => {
      ...
      cy.get('@testRoot').findByRole('button', { name: 'click to show more' }).click()
      cy.tab()
      cy.findByRole('button', { name: 'nothing again' }).should('be.focused')
      cy.tab()
      cy.findByRole('button', { name: 'click to show less' }).should('be.focused').as('focusedElInTrap')
  ...
  describe('demo: nested', () => {
    it('focus is trapped in the innermost trap of nested traps', () => {
      ...
      cy.get('@testRoot').findByRole('button', { name: 'nothing' }).should('be.focused').as('focusedElInInnerTrap')
  ...
  describe('demo: sibling', () => {
    it('focus returns to lastly activated sibling trap when current trap is deactivated', () => {
      ...
      cy.get('@testRoot')
        ...
        .click()
      cy.findByRole('heading', { name: 'focus-trap demo' }).as('outsideEl').click()
  ...
  describe('demo: tif', () => {
    it('when trap is activated, if there is not any tabbable element in the trap, focus-trap will try to focus the element specified by option "fallbackFocus"', () => {
      ...
      cy.get('@testRoot').findByRole('button', { name: 'show focusable button' }).click()
  ...
  describe('demo: input', () => {
    it('if current focused element is already in the trap, focus activation does not change its selection range"', () => {
      ...
      cy.get('@inputElInTrap').then(([input]) => {
        expect((input as HTMLInputElement).selectionStart).to.equal('1'.length)
        expect((input as HTMLInputElement).selectionEnd).to.equal('1'.length)
      })
  ...
  describe('demo: delay', () => {
    it('activates the focus trap when delayInitialFocus is set to true', () => {
      ...
      cy.get('#activate-delay').as('lastlyFocusedElBeforeTrapIsActivated').type('{enter}')
    ...
    it('activates the focus trap when delayInitialFocus is set to false', () => {
      ...
      cy.get('#activate-no-delay').as('lastlyFocusedElBeforeTrapIsActivated').type('{enter}')
      cy.get('#no-delay').should('have.class', 'trap is-active')
      cy.get('#no-delay').should(($div) => {
        expect($div[0].style.opacity).to.equal('1')
      })
      cy.get('#close-button-no-delay').as('hideButtonInTrap').should('have.focus')
  ...
  describe('demo: radio', () => {
    it('radio group in active trap can have its value changed', () => {
      ...
      */
      ...
      cy.get('@testRoot').findByRole('radio', { name: 'c' }).as('radioC').check().should('be.checked')
  ...
  describe('demo: iframe', () => {
    it('focus can be moved freely in the iframe inside of trap', () => {
      ...
      cy.get('@testRoot')
        ...
        .as('lastlyFocusedElBeforeTrapIsActivated')
        .click()
      cy.get('@testRoot').findByRole('button', { name: 'nothing' }).as('firstElInTrap').should('be.focused')
      ...
      */
      ...
      cy.get('@iframeInTrap').then(([iframe]) => {
        ...
        const buttonEl = (iframe as HTMLFormElement).contentDocument.querySelector('button')
  ...
  describe('Click outside go through', () => {
    const activateTrap = (buttonName = 'activate trap') => {
      cy.get('@testRoot')
        .findByRole('button', {
          name: new RegExp(`^${buttonName.replaceAll('/', '\\/')}`),
        })
        .as('lastlyFocusedElementBeforeTrapIsActivated')
        .click()
    }

    const checkTrap = (lastElementInTrapText: string) => {
      // 1st element should be focused
      cy.get('@testRoot').findByRole('link', { name: 'with' }).as('firstElementInTrap').should('be.focused')
    ...
    describe('demo: allowoutsideclick', () => {
      ...
      it('click on outside element should work while keeping focus trapped inside of trap(createOptions.allowOutsideClick = () => true)', () => {
        ...
        cy.get('@testRoot').findByRole('combobox', { name: 'Set allowOutsideClick as:' }).select('function')
    ...
    describe('demo: clickoutsidedeactivates', () => {
      it('traps focus, deactivates on outside click on checkbox and checkbox focused, clickOutsideDeactivates=Boolean, returnFocusOnDeactivate=true', () => {
        ...
        cy.get('#select-returnfocusondeactivate-clickoutsidedeactivates').select('true')
      ...
      it('traps focus, deactivates on outside click on checkbox and checkbox focused, clickOutsideDeactivates=Function, returnFocusOnDeactivate=true', () => {
        ...
        cy.get('@testRoot').findByRole('combobox', { name: 'Set clickOutsideDeactivates as:' }).select('function')
      ...
      it('traps focus, deactivates on outside click on checkbox and checkbox focused, returnFocusOnDeactivate=false', () => {
        ...
        cy.get('#select-returnfocusondeactivate-clickoutsidedeactivates').select('false')
      ...
      it('traps focus, deactivates on outside click on document, and nothing is focused', () => {
        ...
        cy.get('@lastlyFocusedElementBeforeTrapIsActivated').should('not.be.focused')
        cy.get('@lastElementInTrap').should('not.be.focused')
  ...
  describe('demo: setreturnfocus', () => {
    it('specify element to receive focus after trap deactivation', () => {
      ...
      cy.get('@testRoot')
        ...
        .as('lastlyFocusedElBeforeTrapIsActivated')
        .click()
      cy.get('@testRoot').findByRole('link', { name: 'with' }).as('firstElInTrap').should('be.focused')
      verifyCrucialFocusTrapOnClicking('@firstElInTrap')
  ...
  describe('demo: multiple elements passed in', () => {
    ...
    it('can adjust to some containers no longer containing any tabbable nodes', () => {
      ...
      // 1st element should be focused
      cy.get('@testRoot').findByRole('button', { name: 'Gets removed' }).as('firstElementInTrap').should('be.focused')
      ...
      cy.get('@testRoot').findByRole('button', { name: 'Remove button' }).as('removeButton').click()
    ...
    it('can adjust to all containers no longer containing any tabbable nodes provided there is a fallback focus node', () => {
      ...
      cy.get('@testRoot').findByRole('button', { name: 'Remove all button' }).as('removeButton')
  ...
  describe('demo: arrow-keys', () => {
    it('traps focus tab sequence using j/k keys and allows deactivation by clicking deactivate button', () => {
      ...
      cy.focused().should('have.text', 'focusable').trigger('keydown', { key: 'k' })
      cy.focused().as('lastElementInTrap').should('contain', 'deactivate trap').trigger('keydown', { key: 'k' })
      ...
      cy.get('@firstElementInTrap').should('be.focused').trigger('keydown', { key: 'j' })
      cy.get('@lastElementInTrap').should('be.focused')
