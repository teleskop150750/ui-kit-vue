...
export default () => {
  class OpenShadowTest extends HTMLElement {
    constructor() {
      ...
      shadowEl.append(containerEl)
    }
  }

  const createClosedShadow = (hostEl) => {
    const containerEl = document.createElement('div')
  ...
  //  when it requests it
  const closedShadowHostEl = document.querySelector('#with-shadow-dom-closed-shadow')
  const closedShadowEl = createClosedShadow(closedShadowHostEl)

  const containerEl = document.querySelector('#with-shadow-dom')
  ...
  document.querySelector('#activate-with-shadow-dom').addEventListener('click', focusTrap.activate)
  document.querySelector('#deactivate-with-shadow-dom').addEventListener('click', focusTrap.deactivate)
}
