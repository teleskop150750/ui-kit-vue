...
import { getFixtures, removeAllChildNodes, setupTestWindow } from './e2e.helpers'
...
describe('isFocusable', () => {
  ...
  describe('returns true', () => {
    ...
    it('returns true for an `a` anchor element with an `href` attribute', () => {
      ...
      container.innerHTML = '<a href="https://github.com/focus-trap/tabbable">Focusable</a>'
      document.body.append(container)
    ...
    it('returns true for any element with a `contenteditable` attribute with a truthy value', () => {
      ...
      const editableParagraph = getByText(container, 'contenteditable paragraph')
      const editableDivWithNegativeTabIndex = getByText(container, 'contenteditable div focusable but not tabbable')
      const editableDivWithNanTabIndex = getByText(container, 'contenteditable div focusable and tabbable')
      const audioWithNanTabIndex = getByText(container, 'audio controls focusable and tabbable')
      const videoWithNanTabIndex = getByText(container, 'video controls focusable and tabbable')
    ...
    it('returns true for any element with a non-negative `tabindex` attribute', () => {
      ...
      expect(isFocusable(getByText(container, 'Focusable paragraph'))).to.eql(true)
    ...
    it('returns true for any element with a negative `tabindex` attribute', () => {
      ...
      expect(isFocusable(getByTestId(container, 'focusableInput'))).to.eql(true)
      expect(isFocusable(getByText(container, 'Focusable paragraph'))).to.eql(true)
      expect(isFocusable(getByText(container, 'Focusable div'))).to.eql(true)
      expect(isFocusable(getByText(container, 'Focusable span'))).to.eql(true)
    })
  ...
  describe('returns false', () => {
    ...
    it('returns false for any element with a `disabled` attribute', () => {
      ...
      expect(isFocusable(getByTestId(container, 'disabledInput'))).to.eql(false)
    ...
    it('returns false for any element that is visually hidden with a `display: none` style', () => {
      ...
      container.innerHTML = '<input style="display: none;" data-testid="hiddenInput" />'
      document.body.append(container)
    ...
    it('returns false for any element that is visually hidden with a `visibility: hidden` style', () => {
      ...
      container.innerHTML = '<input style="visibility: hidden;" data-testid="hiddenInput" />'
      document.body.append(container)
    ...
    it('returns false for any element that is a descendant of an ancestor that is visually hidden with a `display: none` style', () => {
      ...
      expect(isFocusable(getByTestId(container, 'inputChildOfHiddenAncestor'))).to.eql(false)
    })
    ...
    it('returns false for any non-`summary` element descendants of a closed `details` element', () => {
      ...
      expect(isFocusable(getByTestId(container, 'childInputInClosedDetails'))).to.eql(false)

      expect(isFocusable(getByTestId(container, 'openDetails'))).to.eql(true)
      expect(isFocusable(getByTestId(container, 'childInputInOpenDetails'))).to.eql(true)
    })

    it('returns false for any form element inside a disabled fieldset', () => {
      const container = document.createElement('div')

      container.innerHTML = fixtures.fieldset.replaceAll('id="', 'data-testid="')
      document.body.append(container)

      // in first legend of disabled fieldset: focusable
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend1-button'))).to.eql(true)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend1-input'))).to.eql(true)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend1-select'))).to.eql(true)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend1-textarea'))).to.eql(true)

      // in second (or subsequent) legend of disabled fieldset: NOT focusable
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend2-button'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend2-input'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend2-select'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-legend2-textarea'))).to.eql(false)

      expect(isFocusable(getByTestId(container, 'fieldset-disabled-button'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-input'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-select'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-textarea'))).to.eql(false)

      // nested in enabled fieldset, disabled parent fieldset takes precedence
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-fieldset-enabled-legend-button'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-fieldset-enabled-input'))).to.eql(false)

      // nested in disabled fieldset, top-most disabled fieldset takes precedence
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-fieldset-disabled-legend-button'))).to.eql(false)
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-fieldset-disabled-input'))).to.eql(false)

      // anchor is not form field so it remains focusable
      expect(isFocusable(getByTestId(container, 'fieldset-disabled-anchor'))).to.eql(true)
    })
  ...
  describe('display check', () => {
    ...
    function setupDisplayCheck(inDocument = true) {
      ...
      return {
        ...
        nestedUnderDisplayedNone: getByTestId(container, 'nested-under-displayed-none'),
        displayedContentsTop: getByTestId(container, 'displayed-contents-top'),
        nestedUnderDisplayedContents: getByTestId(container, 'nested-under-displayed-contents'),
      }
    }

    ;[undefined, 'full'].forEach((displayCheck) => {
      ;[true, false].forEach((inDocument) => {
        it(`returns browser visible elements by default ("${displayCheck || '(default)'}" option, container ${
          inDocument ? '' : 'NOT '
        }in doc)`, () => {
          const {
            displayedTop,
          ...
          const options = { displayCheck } as any

          expect(isFocusable(displayedTop, options)).to.eql(!!(inDocument || displayCheck === 'legacy-full'))
          expect(isFocusable(displayedNested, options)).to.eql(!!(inDocument || displayCheck === 'legacy-full'))
          expect(isFocusable(displayedZeroSize, options)).to.eql(!!(inDocument || displayCheck === 'legacy-full'))
          expect(isFocusable(displayedNoneTop, options)).to.eql(
            inDocument || displayCheck === 'legacy-full' ? !inDocument && displayCheck === 'legacy-full' : false,
          )
    ...
    it('returns only elements with size ("non-zero-area" option)', () => {
      ...
      const options = { displayCheck: 'non-zero-area' } as const
    ...
    ;[true, false].forEach((inDocument) => {
      it(`returns elements without checking display ("${inDocument ? 'none' : 'full'}" option, container ${
        inDocument ? 'IN doc' : 'NOT in doc'
      })`, () => {
        const {
          displayedTop,
          displayedNested,
        ...
        const options = { displayCheck: 'none' } as const
