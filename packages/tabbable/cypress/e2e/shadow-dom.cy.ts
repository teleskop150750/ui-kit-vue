...
describe('web-components', () => {
  ...
  describe('isFocusable/isTabbable', () => {
    it('should separate shadow/light radio button groups', () => {
      ...
      const { shadowRoot } = container.querySelector('#webComp')
      ...
      // always focusable
      expect(isFocusable(shadowRadio1), '(focusable) checked in shadow').to.eql(true)
      expect(isFocusable(shadowRadio2), '(focusable) not checked in shadow').to.eql(true)
      expect(isFocusable(lightRadio1), '(focusable) checked in light').to.eql(true)
      expect(isFocusable(radio2Slotted), '(focusable) not checked slotted').to.eql(true)
      expect(isFocusable(lightRadio3), '(focusable) not checked in light').to.eql(true)

      // only the first checked is tabbable
      expect(isTabbable(shadowRadio1), '(tabbable) checked in shadow').to.eql(true)
      expect(isTabbable(shadowRadio2), '(tabbable) not checked in shadow').to.eql(false)
      expect(isTabbable(lightRadio1), '(tabbable) checked in light').to.eql(true)
      expect(isTabbable(radio2Slotted), '(tabbable) not checked slotted').to.eql(false)
      expect(isTabbable(lightRadio3), '(tabbable) not checked in light').to.eql(false)
    })
    ...
    it('should not match elements in shadow root with a "display=none" ancestor', () => {
      ...
      const { shadowRoot } = container.querySelector('test-shadow')
      ...
      // focusable
      expect(isFocusable(shadowInput), '(focusable) non display shadow').to.eql(false)
      expect(isFocusable(lightInputSlotted), '(focusable) non display slotted').to.eql(false)

      // tabbable
      expect(isTabbable(shadowInput), '(tabbable) non display shadow').to.eql(false)
      expect(isTabbable(lightInputSlotted), '(tabbable) non display slotted').to.eql(false)
    })
    ...
    it('should not match elements in a closed shadow root with inner display="none" (fallback to zero-area-size)', () => {
      ...
      const lightDisplayNoneSlotted = container.querySelector('#light-input-slotted')

      // focusable
      expect(isFocusable(lightDisplayNoneSlotted), 'slotted into non-displayed container so not focusable').to.eql(
        false,
      )
      expect(
      ...
      // tabbable
      expect(isTabbable(lightDisplayNoneSlotted), 'slotted into non-displayed container so not tabbable').to.eql(false)
      expect(
    ...
    it('should not match slot elements', () => {
      ...
      const { shadowRoot } = container.querySelector('test-shadow')
      const slotWithTabIndex = shadowRoot.querySelector('slot[name="slot-first"]')
  ...
  describe('tabbable/focusable', () => {
    describe('common', () => {
      it('should not find elements inside shadow dom that browsers will skip due to -1 tabindex on host', () => {
        ...
        let result = tabbable(shadowElement, { getShadowRoot() {} } as any)

        expect(getIdsFromElementsArray(result), 'using `() => {}`').to.eql(expected)
      ...
      it('should sort shadow and light elements separately', () => {
        const expected = ['light-first', 'light-middle', 'light-last', 'shadow-first', 'shadow-middle', 'shadow-last']
        const { container } = setupFixture(fixtures.shadowDomQuery, {
          window,
          caseId: 'light-shadow-tab-index',
        })

        let result = tabbable(container, { getShadowRoot() {} } as any)

        expect(getIdsFromElementsArray(result), 'using `() => {}`').to.eql(expected)

        result = tabbable(container, { getShadowRoot() {} } as any)
        expect(getIdsFromElementsArray(result), 'using `true`').to.eql(expected)
      })
      ...
      it('should sort slots content by slots tabindex', () => {
        ...
        let tabbableList = tabbable(container, { getShadowRoot() {} } as any)
        let focusableList = focusable(container, { getShadowRoot() {} } as any)

        expect(getIdsFromElementsArray(tabbableList), 'tabbable, using `() => {}`').to.eql([
          'light-1',
          'shadow-1',
          'shadow-2',
          'light-2',
        ...
        expect(getIdsFromElementsArray(focusableList), 'focusable, using `() => {}`').to.eql([
          'shadow-3',
          'light-3',
          'light-2',
          'light-1',
        ...
        expect(getIdsFromElementsArray(tabbableList), 'tabbable, using `true`').to.eql([
          'light-1',
          'shadow-1',
          'shadow-2',
          'light-2',
        ...
        expect(getIdsFromElementsArray(focusableList), 'focusable, using `true`').to.eql([
          'shadow-3',
          'light-3',
          'light-2',
          'light-1',
      ...
      it('should insert slots nested content according to slot positions', () => {
        const expected = ['light-1', 'light-2', 'light-3', 'shadow-between', 'light-4', 'light-5', 'light-6']
        const { container } = setupFixture(fixtures.shadowDomQuery, {
          window,
      ...
      it('should not find slot with tabindex', () => {
        ...
        const shadowElement = container.querySelector('test-shadow')
        const slot = shadowElement.shadowRoot.querySelector('slot[name="before"]')

        slot.id = 'slot-id'
        slot.tabIndex = '1'

        let result = tabbable(slot, { getShadowRoot() {} } as any)
      ...
      it('should filter un-tabbable elements', { browser: '!firefox' }, () => {
        const expected = ['shadow-summary', 'shadow-details-without-summary', 'shadow-radio-checked']
        const { container } = setupFixture(fixtures.shadowDomQuery, {
          window,
      ...
      it('should filter un-focusable elements', { browser: '!firefox' }, () => {
        ...
        let result = focusable(container, { getShadowRoot() {} } as any)
      ...
      it('should not match elements in a non display slot', () => {
        ...
        expect(getIdsFromElementsArray(focusable(container))).to.eql(expected)

        expect(
          getIdsFromElementsArray(
            focusable(container, {
              getShadowRoot(node: any) {
        ...
        expect(getIdsFromElementsArray(tabbable(container))).to.eql(expected)
    ...
    ;[false, true].forEach((inertTest) => {
      // TODO[ff-inert-support]: FF does not yet (Feb 2023) support the `inert` attribute
      describe(`${inertTest ? 'inertness' : 'activeness'}`, { browser: inertTest ? '!firefox' : undefined }, () => {
        it(
          `should${inertTest ? ' not' : ''} find${inertTest ? ' inert' : ''} elements inside shadow dom`,
          { browser: '!chrome' },
          () => {
        ...
        it(`should${inertTest ? ' not' : ''} find${inertTest ? ' inert' : ''} tabbable host`, () => {
          const expected = inertTest
            ? ['light-before', 'light-after']
        ...
        it(`should${inertTest ? ' not' : ''} find${inertTest ? ' inert' : ''} tabbable scoped headers`, () => {
          const expected = inertTest
            ? []
            : ['header-athlete-col', 'header-swim-col', 'header-bike-col', 'header-run-col']
          const { container } = setupFixture(fixtures.shadowDomQuery, {
            window,
            caseId: inertTest ? 'table-with-inert-tabbable-scoped-headers' : 'table-with-tabbable-scoped-headers',
          })
        ...
        it(`should${inertTest ? ' not' : ''} find elements inside shadow dom when directly querying the${
          inertTest ? ' inert' : ''
        ...
        //  ends-up inert too)
        it(`should${inertTest ? ' ignore inert' : ' sort'} slots inside shadow dom`, () => {
          const expected = inertTest
            ? ['light-before', 'shadow-input', 'light-after']
            : [
                'light-before',
          ...
          const { container } = setupFixture(fixtures.shadowDomQuery, {
            window,
            caseId: inertTest ? 'light-shadow-with-inert-slots' : 'light-shadow-with-slots',
          })
        ...
        it(`should query nested shadow doms for slots${inertTest ? ' (inert test)' : ''}`, () => {
          const expected = inertTest
            ? ['shadow-outter-before', 'shadow-inner', 'shadow-outter-after']
            : ['shadow-outter-before', 'shadow-inner', 'light-slotted-input', 'shadow-outter-after']

          const { container } = setupFixture(fixtures.shadowDomQuery, {
            window,
            caseId: inertTest ? 'inert-slotted-through-multiple-shadows' : 'slotted-through-multiple-shadows',
          })
        ...
        it(`should${inertTest ? ' not' : ''} find elements inside${inertTest ? ' inert' : ''} slot`, () => {
        ...
        it(`should accept shadow root in order to query closed shadows${inertTest ? ' (inert test)' : ''}`, () => {
          const { container } = setupFixture(fixtures.shadowDomQuery, {
            window,
            caseId: inertTest ? 'closed-inert-shadow-input' : 'closed-shadow-input',
          })

          const providedShadowRoot = tabbable(container, {
            getShadowRoot(node) {
              return (node as any).closedShadowRoot
            },
          })
          ...
          expect(getIdsFromElementsArray(providedShadowRoot), 'provided shadow root').to.eql([
            'light-before',
          ...
          expect(getIdsFromElementsArray(undisclosedShadowRoot), 'undisclosed shadow root').to.eql([
            'light-before',
            'light-slotted',
            'light-after',
          ])
