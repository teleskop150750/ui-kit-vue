...
import { getFixtures, getIdsFromElementsArray, setupFixture, setupTestWindow } from './e2e.helpers'
...
describe('focusable', () => {
  ...
  before(() => {
    getFixtures((f: Record<string, string>) => {
      fixtures = f
    })
  })

  beforeEach(() => {
    setupTestWindow((testWindow: Window) => {
  ...
  describe('example fixtures', () => {
    ;[undefined, 'full'].forEach((displayCheck) => {
      ;[true, false].forEach((inDocument) => {
        it(`correctly identifies focusable elements in the "basic" example ${
          ...
          const focusableElements = focusable(container, { displayCheck } as any)

          expect(getIdsFromElementsArray(focusableElements)).to.eql(expectedFocusableIds)
        })
      })
    })
    ...
    describe('inertness', { browser: '!firefox' }, () => {
      ;[undefined, 'full', 'non-zero-area', 'none'].forEach((displayCheck) => {
    ...
    it('correctly identifies focusable elements in the "changing content" example', () => {
      ...
      expect(getIdsFromElementsArray(focusableElementsAfterSectionIsUnhidden)).to.eql(
    ...
    it('correctly identifies focusable elements in the "shadow-dom" example', () => {
      ...
      const focusableElements = focusable(host.shadowRoot.querySelector('#container'))
    ...
    describe('options argument', () => {
      ...
      describe('displayed check', () => {
        it('return browser visible elements by default ("full" option)', () => {
          ...
          expect(getIdsFromElementsArray(focusableElementsDefault)).to.eql(expectedFocusableIds)
        ...
        it('return only elements with size ("non-zero-area" option)', () => {
          ...
          expect(getIdsFromElementsArray(focusableElementsWithSize)).to.eql(expectedFocusableIds)
        })
