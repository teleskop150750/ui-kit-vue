...
describe('tabbable', () => {
  ...
  describe('example fixtures', () => {
    ;[undefined, 'full'].forEach((displayCheck) => {
      ;[true, false].forEach((inDocument) => {
        it(`correctly identifies tabbable elements in the "basic" example ${
          ...
          const expectedTabbableIds: string[] = inDocument
            ? [
          ...
          const tabbableElements = tabbable(container, { displayCheck } as any)
    ...
    describe('inertness', { browser: '!firefox' }, () => {
      ;[undefined, 'full', 'legacy-full', 'non-zero-area', 'none'].forEach((displayCheck) => {
    ...
    it('correctly identifies tabbable elements in the "changing content" example', () => {
      const expectedTabbableIds = ['visible-button-1', 'visible-button-2', 'visible-button-3']
      ...
      expect(getIdsFromElementsArray(tabbableElementsAfterSectionIsUnhidden)).to.eql(
    ...
    it('correctly identifies tabbable elements in the "details" example', () => {
      const expectedTabbableIds = ['details-a-summary', 'details-b-summary', 'visible-input', 'details-c']
    ...
    it('correctly identifies tabbable elements in the "shadow-dom" example', () => {
      ...
      const tabbableElements = tabbable(host.shadowRoot.querySelector('#container'))
    ...
    describe('options argument', () => {
      it('includes the container element when the `includeContainer` property is true', () => {
        const expectedTabbableIds = ['tabindex-div-2', 'container-div', 'tabindex-div-0', 'input']
      ...
      describe('displayed check', () => {
        it('return browser visible elements by default ("full" option)', () => {
          ...
          expect(getIdsFromElementsArray(tabbableElementsDefault)).to.eql(expectedTabbableIds)
          expect(getIdsFromElementsArray(tabbableElementsFull)).to.eql(getIdsFromElementsArray(tabbableElementsDefault))
        })
        it('return only elements with size ("non-zero-area" option)', () => {
          const expectedTabbableIds = ['displayed-top', 'displayed-nested', 'nested-under-displayed-contents']
          const container = document.createElement('div')
          ...
          expect(getIdsFromElementsArray(tabbableElementsWithSize)).to.eql(expectedTabbableIds)
        })
