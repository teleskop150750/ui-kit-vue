...
describe('<dynamic-size-list />', () => {
  ...
  describe('scroll functionality', () => {
    it("should update inner container's height after scroll dispatched", async () => {
      ...
      expect(estimatedTotalSize).toBe(BASE_SIZE)
      ...
      await nextTick()
      expect(Number.parseInt(listRef.innerRef.style.height)).toBe(estimatedTotalSize)
    ...
    it('should scroll correctly in horizontal mode', async () => {
      ...
      listRef.scrollTo(200)
      await nextTick()
      expect(Number.parseInt(listRef.innerRef.style.width)).toBe(estimatedTotalSize)
      // when using scrollTo method, the list consists of
      ...
      expect(Number.parseInt(listRef.innerRef.style.width)).toBe(BASE_SIZE)
      expect(Number.parseInt(listRef.innerRef.style.width)).toBe(BASE_SIZE)
  ...
  describe('to throw', () => {
    it('should throw when item-size is not function', () => {
      ...
      try {
        ...
        expect((error as Error).message).toContain('itemSize is required as function')
      }
    })
  })
})
