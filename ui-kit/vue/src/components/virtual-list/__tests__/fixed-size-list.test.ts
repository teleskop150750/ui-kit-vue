...
import { makeMount } from '@ui/test-utils/make-mount'
import { makeScroll } from '@ui/test-utils/make-scroll'
...
const mount = makeMount(
  {
    template: `<NVirtualList v-bind="$attrs" ref="listRef">
...
describe('<fixed-size-list />', () => {
  ...
  it('should render correctly', async () => {
    ...
    expect(wrapper.find(WINDOW_SELECTOR).element.firstElementChild!.getAttribute('style')).toBe(
  ...
  it('should early terminate scroll handler when the scrollTo equals to current offset', async () => {
    ...
    // @ts-expect-error
    await makeScroll((wrapper.vm.$refs.listRef as ListRef).containerRef, 'scrollTop', 0)
    expect(wrapper.find(ITEM_SELECTOR).text()).toContain(0)
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    await makeScroll((wrapper.vm.$refs.listRef as ListRef).containerRef, 'scrollLeft', 0)
  ...
  it('should render correct items to the dom', async () => {
    ...
    // container ref has onScroll handler so we trigger scroll on containerRef

    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    const { containerRef } = wrapper.vm.$refs.listRef as ListRef

    expect(wrapper.findAll(ITEM_SELECTOR)).toHaveLength(7)
    expect(onItemRendered).toHaveBeenCalledTimes(1)

    makeScroll(containerRef, 'scrollTop', 100)
    await nextTick()
  ...
  it('should handle horizontal scroll correctly', async () => {
    ...
    makeScroll(containerRef, 'scrollLeft', 100)
    await nextTick()
  ...
  it('should render rtl direction', async () => {
    ...
    await nextTick()
    expect(wrapper.find(WINDOW_SELECTOR).element.getAttribute('style')!.includes(`direction: ${RTL}`)).toBe(true)
