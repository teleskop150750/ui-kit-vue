import { isFirefox, isNumber, type Nillable } from '@nado/ui-kit-utils'
...
function calculateNodeStyling(targetElement: Element): NodeStyle {
  ...
  const paddingSize = paddingBottom + paddingTop
  ...
  const borderSize = borderBottomWidth + borderTopWidth
...
export function calcTextareaHeight(
  ...
  minRows = 1,
  maxRows: Nillable<number> = undefined,
): TextAreaHeight {
  if (!hiddenTextarea) {
