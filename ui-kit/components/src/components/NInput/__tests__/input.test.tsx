...
describe('Input.vue', () => {
  ...
  test('create', async () => {
    ...
    await inputElm.trigger('focusin')
  ...
  describe('test emoji', () => {
    test('el-input should minimize value between emoji length and maxLength', async () => {
      ...
      const elCount = wrapper.find('.n-input__count-inner')
      ...
      expect([...vm.$el.classList]).toMatchInlineSnapshot(['n-input', 'n-input--is-exceed', 'test-exceed'])
    })
    ...
    test('textarea should minimize value between emoji length and maxLength', async () => {
      ...
      expect(nativeInput.value).toMatchSnapshot('啊好😄')

      const elCount = wrapper.find('.n-textarea__count')

      expect(elCount.exists()).toBe(true)
      expect(elCount.text()).toMatchSnapshot('4 / 4')

      inputVal.value = '哈哈1👌3😄'
      await nextTick()
      expect(nativeInput.value).toMatchSnapshot('哈哈1👌3😄')
      expect(elCount.text()).toMatchSnapshot('8 / 4')
      expect([...vm.$el.classList]).toMatchSnapshot(['n-textarea', 'n-textarea--is-exceed'])
    })
  })
  ...
  test('sets value on textarea / input type change', async () => {
    ...
    expect(vm.$el.querySelector('textarea').value).toMatchSnapshot('"123"')
    type.value = 'password'
    await nextTick()
    await nextTick()
  ...
  test('limit input and show word count', async () => {
    ...
    expect(inputElm2.querySelectorAll('.n-textarea__count').length).toEqual(1)
    expect(inputElm3.querySelectorAll('.n-input__count').length).toEqual(0)

    expect([...inputElm4.classList]).toMatchInlineSnapshot(['n-input', 'n-input--is-exceed', 'test-initial-exceed'])
    ...
    input4.value = '1'
    await nextTick()
    expect([...inputElm4.classList]).toMatchInlineSnapshot(['n-input', 'test-initial-exceed'])
  })
  ...
  describe('Input Methods', () => {
    test('method:select', async () => {
      ...
      defineGetter(input, 'selectionStart', function f(this: HTMLInputElement) {
        // eslint-disable-next-line no-invalid-this
        return selected ? 0 : this.value.length
      })
      defineGetter(input, 'selectionEnd', function f(this: HTMLInputElement) {
        // eslint-disable-next-line no-invalid-this
    ...
    test('method:resizeTextarea', async () => {
      ...
      ;(refTextarea.autosize as Exclude<NInputAutoSize, boolean>).minRows = 5
  ...
  describe('Input Events', () => {
    ...
    test('event:clear', async () => {
      ...
      await input.trigger('focusin')
      await nextTick()
      vm.$el.querySelector('.n-input__clear').click()
      await nextTick()
