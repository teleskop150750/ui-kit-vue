...
import NRovingFocusGroup from '../src/NRovingFocusGroup.vue'
import { ROVING_FOCUS_COLLECTION_INJECTION_KEY } from '../src/roving-focus-group.model'
import { ROVING_FOCUS_GROUP_INJECTION_KEY } from '../src/tokens'

const AXIOM = 'rem is the best girl'
const RovingFocusGroupChild = defineComponent({
  setup() {
    const rovingFocusGroupInjection = inject(ROVING_FOCUS_GROUP_INJECTION_KEY, undefined)!

    const { itemMap, collectionRef } = inject(ROVING_FOCUS_COLLECTION_INJECTION_KEY, undefined)!
    ...
    const composedRef = composeRefs(rovingFocusGroupInjection.rovingFocusGroupRef, collectionRef)
...
describe('<NRovingFocusGroup />', () => {
  ...
  it('should be able to control current tab', async () => {
    ...
    expect(child.vm.currentTabbedId).toBe(undefined)
