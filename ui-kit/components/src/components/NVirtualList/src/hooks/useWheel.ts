import { cAF, isFirefox, rAF } from '@nado/ui-kit-utils'
...
export const useWheel = ({ atEndEdge, atStartEdge, layout }: ListWheelState, onWheelDelta: ListWheelHandler) => {
  ...
  const hasReachedEdge = (offsetVal: number) => {
    const edgeReached = (offsetVal < 0 && atStartEdge.value) || (offsetVal > 0 && atEndEdge.value)
