...
import { getStyle } from '@nado/ui-kit-utils'
import { h, nextTick, ref } from 'vue'

import { NMessage } from '..'
import { Message } from '../src/message'

const selector = '.n-message'
...
describe('Message on command', () => {
  ...
  test('it should close all messages of the specified type', async () => {
    ...
    await rAF()
    const elements = document.querySelectorAll(selector)
    const successElements = document.querySelectorAll(`${selector}--appearance-${success}`)
  ...
  test('it should stack messages', async () => {
    ...
    const getTopValue = (elm: Element): number => Number.parseInt(getStyle(elm as HTMLElement, 'top'))
  ...
  test('correct space when set offset', async () => {
    ...
    const getTopValue = (elm: Element): number => Number.parseFloat(getStyle(elm as HTMLElement, 'top'))
  ...
  test('it should have 4 other types of message', () => {
    ...
    expect(Message.danger).toBeInstanceOf(Function)
  })
  ...
  test('it should render vnode function', async () => {
    ...
    await rAF()
    expect(document.querySelector(selector)?.textContent).toMatchInlineSnapshot(`"0"`)

    i.value += 1

    await rAF()
    expect(document.querySelector(selector)?.textContent).toMatchInlineSnapshot(`"1"`)
  })

  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(NMessage._context).toBe(null)
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      NMessage.install?.(testContext as any)
      expect(NMessage._context).not.toBe(null)
      expect(NMessage._context).toBe(testContext._context)
      // clean up
      NMessage._context = null
    })
  })
})
