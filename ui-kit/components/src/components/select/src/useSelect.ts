...
import { type NTooltip } from '../../tooltip'
...
export const useSelect = (props, states: States, ctx) => {
  ...
  const tooltipRef = ref<InstanceType<typeof NTooltip> | null>(null)
  ...
  const emptyText = computed(() => {
    if (props.loading) {
      return props.loadingText || t('nado.select.loading')
    }
    ...
    if (props.filterable && states.query && states.options.size > 0 && states.filteredOptionsCount === 0) {
      return props.noMatchText || t('nado.select.noMatch')
    }

    if (states.options.size === 0) {
      return props.noDataText || t('nado.select.noData')
    }
  ...
  const optionsArray = computed(() => {
    const list = [...states.options.values()]
    const newList = []
    ...
    (val, oldVal) => {
      if (props.multiple) {
        ...
        states.currentPlaceholder =
          (val && val.length > 0) || (input.value && states.query !== '') ? '' : states.cachedPlaceHolder
    ...
    (val) => {
      if (!val) {
        ...
        if (!props.multiple) {
          if (states.selected) {
            states.selectedLabel =
              props.filterable && props.allowCreate && states.createdSelected && states.createdLabel
                ? states.createdLabel
                : states.selected.currentLabel
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const getValueIndex = (arr: any[] = [], value = undefined) => {
    if (!isObject(value)) {
  ...
  const scrollToOption = (option) => {
    ...
    if (tooltipRef.value && target) {
      const menu = tooltipRef.value?.popperRef?.contentRef?.querySelector?.(`.${ns.se('dropdown', 'wrap')}`)
  ...
  const getValueKey = (item) => (isObject(item.value) ? get(item.value, props.valueKey) : item.value)
