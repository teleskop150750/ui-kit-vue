...
describe('virtual scrollbar', () => {
  ...
  it('click track', async () => {
     ...
     */
    const initializeStyle = 'height: 33px; transform: translateY(0px); webkit-transform: translateY(0px); width: 100%;'

    expect(wrapper.find('.n-virtual-scrollbar__thumb').attributes('style')).toContain(initializeStyle)
    ...
    expect(wrapper.find('.n-virtual-scrollbar__thumb').attributes('style')).not.toContain(initializeStyle)
  })
  ...
  it('horizontal track height/width', async () => {
    ...
    expect((wrapper.find('.n-virtual-scrollbar').element as HTMLElement).style.width).toContain('200px') // clientSize - props.endGap = 200 - 2 = 198

    expect((wrapper.find('.n-virtual-scrollbar').element as HTMLElement).style.height).toContain('12px') // fixed 6
  })
  ...
  it('vertical track height/width', async () => {
    ...
    expect((wrapper.find('.n-virtual-scrollbar').element as HTMLElement).style.height).toContain('100px') // clientSize - props.endGap = 100 - 2 = 98

    expect((wrapper.find('.n-virtual-scrollbar').element as HTMLElement).style.width).toContain('12px') // fixed 6
  })
  ...
  it('should locate correctly when endGap is set', async () => {
    ...
    expect((wrapper.find('.n-virtual-scrollbar').element as HTMLElement).style.height).toContain('100px') // clientSize - props.endGap = 100 - 8 = 92
  })
})
